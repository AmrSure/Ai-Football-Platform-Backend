...
python manage.py migrate --settings=config.settings.production_deploy
...
if ! python manage.py shell --settings=config.settings.production_deploy -c "
...
" 2>/dev/null; then
    ...
    python manage.py createsuperuser --settings=config.settings.production_deploy --noinput || true
else
...
" 2>/dev/null; then
    ...
    python manage.py shell --settings=config.settings.production_deploy -c "
from apps.core.models import User
...
python manage.py check --settings=config.settings.production_deploy --deploy

# Test database connection
print_status "Testing database connection..."
python manage.py dbshell --settings=config.settings.production_deploy -c "SELECT 1;" > /dev/null 2>&1
